...
<div class="container mx-auto py-8 max-w-3xl">
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        <div class="bg-green-100 dark:bg-green-900/30 rounded-xl p-6 shadow border border-green-200 dark:border-green-800 flex flex-col items-center">
            <div class="text-2xl font-bold text-green-700 dark:text-green-300 mb-1">R$ {{ number_format($totalRecebido, 2, ',', '.') }}</div>
            <div class="text-sm text-green-900 dark:text-green-200 font-semibold">Total Recebido</div>
        </div>
        <div class="bg-yellow-100 dark:bg-yellow-900/30 rounded-xl p-6 shadow border border-yellow-200 dark:border-yellow-800 flex flex-col items-center">
            <div class="text-2xl font-bold text-yellow-700 dark:text-yellow-300 mb-1">R$ {{ number_format($totalPendente, 2, ',', '.') }}</div>
            <div class="text-sm text-yellow-900 dark:text-yellow-200 font-semibold">Total Pendente</div>
        </div>
        <div class="bg-blue-100 dark:bg-blue-900/30 rounded-xl p-6 shadow border border-blue-200 dark:border-blue-800 flex flex-col items-center">
            <div class="text-2xl font-bold text-blue-700 dark:text-blue-300 mb-1">R$ {{ number_format($totalGeral, 2, ',', '.') }}</div>
            <div class="text-sm text-blue-900 dark:text-blue-200 font-semibold">Total Geral</div>
        </div>
    </div>
    <div class="mb-8">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow border border-gray-200 dark:border-gray-800 p-4">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <button type="button" onclick="document.getElementById('modalDiariaRapida').classList.remove('hidden')" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition flex items-center gap-2">
            <i class="fa-solid fa-bolt"></i> Lançar Diária Rápida
        </button>
    </div>
    @if(session('success'))
...
<div id="modalDiariaRapida" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
        ...
        <form method="POST" action="{{ route('daily-forecasts.store') }}">
            ...
            <div class="mb-4">
                ...
                <select name="client_id" class="w-full rounded px-3 py-2 border border-gray-300 text-gray-900">
...
<div id="modalReceberDiaria" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
