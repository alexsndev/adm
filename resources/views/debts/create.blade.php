    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white/60 dark:bg-gray-900/60 rounded-3xl shadow-xl p-8 border border-white/30 dark:border-gray-700/30 backdrop-blur-lg animate-fade-in">
            <form method="POST" action="{{ route('debts.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="debt_type" id="debt_type" required onchange="toggleCreditCardField()"
                            ...
                            <option value="personal_loan" {{ old('debt_type') == 'personal_loan' ? 'selected' : '' }}>Empréstimo Pessoal</option>
                            <option value="mortgage" {{ old('debt_type') == 'mortgage' ? 'selected' : '' }}>Financiamento Imobiliário</option>
                            <option value="car_loan" {{ old('debt_type') == 'car_loan' ? 'selected' : '' }}>Financiamento de Veículo</option>
                            <option value="student_loan" {{ old('debt_type') == 'student_loan' ? 'selected' : '' }}>Empréstimo Estudantil</option>
                            <option value="business_loan" {{ old('debt_type') == 'business_loan' ? 'selected' : '' }}>Empréstimo Empresarial</option>
                    ...
                    <div id="credit_card_field" style="display: none;">
                        ...
                        <select name="credit_card_id" id="credit_card_id"
                            ...
                            @foreach($creditCards as $card)
                                <option value="{{ $card->id }}" data-logo="{{ $card->logo_url }}" {{ old('credit_card_id') == $card->id ? 'selected' : '' }}>
                                    {{ $card->display_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="original_amount" id="original_amount" value="{{ old('original_amount') }}" step="0.01" min="0" required
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200">
                    ...
                    <div>
                        ...
                        <input type="number" name="interest_rate" id="interest_rate" value="{{ old('interest_rate', 0) }}" step="0.01" min="0" max="100" required
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200">
                        @error('interest_rate')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="iof_rate" id="iof_rate" value="{{ old('iof_rate', 0.000082) }}" step="0.000001" min="0" max="1"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200">
                    ...
                    <div>
                        ...
                        <input type="text" name="creditor_contact" id="creditor_contact" value="{{ old('creditor_contact') }}"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200">
                ...
                <!-- Seção de Negociação -->
                <div class="mt-8 p-6 bg-orange-50 dark:bg-orange-900/20 rounded-2xl border border-orange-200 dark:border-orange-700">
                    <h3 class="text-lg font-extrabold text-orange-800 dark:text-orange-200 mb-4 flex items-center">
                        <i class="fa-solid fa-handshake mr-2"></i> Negociação/Acordo
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <!-- Valor Negociado -->
                        <div>
                            <label for="negotiated_amount" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                                <i class="fa-solid fa-percentage mr-1 text-orange-400"></i> Valor Negociado
                            </label>
                            <input type="number" name="negotiated_amount" id="negotiated_amount" value="{{ old('negotiated_amount') }}" step="0.01" min="0"
                                ...
                                placeholder="Valor após desconto">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Valor após desconto/negociação</p>
                        </div>

                        <!-- Número de Parcelas -->
                        <div>
                            <label for="installments" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                                <i class="fa-solid fa-list-ol mr-1 text-orange-400"></i> Parcelas
                            </label>
                            <input type="number" name="installments" id="installments" value="{{ old('installments') }}" min="1" max="120"
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200"
                                placeholder="Número de parcelas">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Quantidade de parcelas</p>
                        </div>

                        <!-- Valor da Parcela -->
                        <div>
                            <label for="installment_amount" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                                <i class="fa-solid fa-coins mr-1 text-orange-400"></i> Valor da Parcela
                            </label>
                            <input type="number" name="installment_amount" id="installment_amount" value="{{ old('installment_amount') }}" step="0.01" min="0"
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200"
                                placeholder="Valor de cada parcela">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Valor de cada parcela</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        <label for="agreement_date" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                            <i class="fa-solid fa-calendar-check mr-1 text-orange-400"></i> Data do Acordo
                        </label>
                        <input type="date" name="agreement_date" id="agreement_date" value="{{ old('agreement_date') }}"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200">
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Data em que foi fechado o acordo</p>
                    </div>
                </div>
                ...
                <div class="mt-8">
                    ...
                    <input type="date" name="first_payment_date" id="first_payment_date" value="{{ old('first_payment_date') }}"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-6">
                    ...
                    <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-bold rounded-xl text-white bg-gradient-to-r from-red-500 to-orange-600 hover:from-red-600 hover:to-orange-700 shadow transition-all duration-200">
                        <i class="fa-solid fa-save mr-2"></i> Criar Dívida
                    </button>
                </div>
            </form>
        </div>
