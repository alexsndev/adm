    ...
    @section('header')
        <div class="flex justify-between items-center mb-2">
            <h2 class="font-extrabold text-3xl md:text-4xl text-gray-900 dark:text-white tracking-tight drop-shadow-lg animate-fade-in">
                <i class="fa-solid fa-edit mr-2 text-green-400"></i> Editar Meta Verde
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white/60 dark:bg-gray-900/60 rounded-3xl shadow-xl p-8 border border-white/30 dark:border-gray-700/30 backdrop-blur-lg animate-fade-in">
                <form method="POST" action="{{ route('financial-goals.update', $financialGoal) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $financialGoal->name) }}" required
                        ...
                        <div>
                            ...
                            <select name="goal_type" id="goal_type" required
                                ...
                                <option value="emergency_fund" {{ old('goal_type', $financialGoal->goal_type) == 'emergency_fund' ? 'selected' : '' }}>Fundo de Emergência</option>
                                <option value="debt_free" {{ old('goal_type', $financialGoal->goal_type) == 'debt_free' ? 'selected' : '' }}>Livre de Dívidas</option>
                                <option value="savings" {{ old('goal_type', $financialGoal->goal_type) == 'savings' ? 'selected' : '' }}>Poupança</option>
                                <option value="investment" {{ old('goal_type', $financialGoal->goal_type) == 'investment' ? 'selected' : '' }}>Investimentos</option>
                                <option value="eco_friendly_home" {{ old('goal_type', $financialGoal->goal_type) == 'eco_friendly_home' ? 'selected' : '' }}>Casa Sustentável</option>
                                <option value="solar_panels" {{ old('goal_type', $financialGoal->goal_type) == 'solar_panels' ? 'selected' : '' }}>Painéis Solares</option>
                                <option value="electric_vehicle" {{ old('goal_type', $financialGoal->goal_type) == 'electric_vehicle' ? 'selected' : '' }}>Veículo Elétrico</option>
                                <option value="sustainable_business" {{ old('goal_type', $financialGoal->goal_type) == 'sustainable_business' ? 'selected' : '' }}>Negócio Sustentável</option>
                                <option value="green_investment" {{ old('goal_type', $financialGoal->goal_type) == 'green_investment' ? 'selected' : '' }}>Investimento Verde</option>
                                <option value="education" {{ old('goal_type', $financialGoal->goal_type) == 'education' ? 'selected' : '' }}>Educação</option>
                                <option value="travel" {{ old('goal_type', $financialGoal->goal_type) == 'travel' ? 'selected' : '' }}>Viagem</option>
                                <option value="retirement" {{ old('goal_type', $financialGoal->goal_type) == 'retirement' ? 'selected' : '' }}>Aposentadoria</option>
                                <option value="other" {{ old('goal_type', $financialGoal->goal_type) == 'other' ? 'selected' : '' }}>Outro</option>
                            </select>
                            @error('goal_type')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="target_amount" id="target_amount" value="{{ old('target_amount', $financialGoal->target_amount) }}" step="0.01" min="0" required
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                            @error('target_amount')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="current_amount" id="current_amount" value="{{ old('current_amount', $financialGoal->current_amount) }}" step="0.01" min="0"
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                        ...
                        <div>
                            ...
                            <select name="priority" id="priority" required
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                                <option value="low" {{ old('priority', $financialGoal->priority) == 'low' ? 'selected' : '' }}>Baixa</option>
                                <option value="medium" {{ old('priority', $financialGoal->priority) == 'medium' ? 'selected' : '' }}>Média</option>
                                <option value="high" {{ old('priority', $financialGoal->priority) == 'high' ? 'selected' : '' }}>Alta</option>
                                <option value="critical" {{ old('priority', $financialGoal->priority) == 'critical' ? 'selected' : '' }}>Crítica</option>
                            </select>
                            @error('priority')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $financialGoal->start_date->format('Y-m-d')) }}" required
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                            @error('start_date')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="target_date" id="target_date" value="{{ old('target_date', $financialGoal->target_date->format('Y-m-d')) }}" required
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                            @error('target_date')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="monthly_contribution" id="monthly_contribution" value="{{ old('monthly_contribution', $financialGoal->monthly_contribution) }}" step="0.01" min="0"
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                        ...
                        <div>
                            ...
                            <select name="account_id" id="account_id"
                                ...
                                @foreach($accounts as $account)
                                    <option value="{{ $account->id }}" {{ old('account_id', $financialGoal->account_id) == $account->id ? 'selected' : '' }}>
                                        {{ $account->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="flex items-center space-x-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-700">
                        <input type="checkbox" name="is_eco_friendly" id="is_eco_friendly" value="1" {{ old('is_eco_friendly', $financialGoal->is_eco_friendly) ? 'checked' : '' }}
                    ...
                    <div id="eco_impact_section" class="{{ old('is_eco_friendly', $financialGoal->is_eco_friendly) ? '' : 'hidden' }}">
                        ...
                        <textarea name="eco_impact_description" id="eco_impact_description" rows="3"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">{{ old('eco_impact_description', $financialGoal->eco_impact_description) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">{{ old('description', $financialGoal->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end space-x-4 pt-6">
                        ...
                        <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-bold rounded-xl text-white bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 shadow transition-all duration-200">
                            <i class="fa-solid fa-save mr-2"></i> Atualizar Meta Verde
                        </button>
                    </div>
                </form>
            </div>
