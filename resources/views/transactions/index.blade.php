...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-blue-900 to-gray-900 py-8">
    <div class="container mx-auto max-w-5xl px-4">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div class="flex gap-2">
                <a href="{{ route('transactions.create') }}" class="inline-flex items-center px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105">
                    <i class="fa-solid fa-plus mr-2"></i> Nova Transação
                </a>
                <a href="{{ route('categories.index') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105">
            ...
            <div class="p-6 w-full max-w-full">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-blue-900">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Descrição</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Categoria</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Conta</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Valor</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Ações</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 transition">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-3">
                                                <a href="{{ route('transactions.show', $transaction) }}" class="px-2 py-1 rounded bg-blue-600 hover:bg-blue-700 text-white font-bold transition shadow flex items-center gap-1" title="Ver"><i class="fa-solid fa-eye"></i> Ver</a>
                                                <a href="{{ route('transactions.edit', $transaction) }}" class="px-2 py-1 rounded bg-green-600 hover:bg-green-700 text-white font-bold transition shadow flex items-center gap-1" title="Editar"><i class="fa-solid fa-pen"></i> Editar</a>
                                                <form action="{{ route('transactions.destroy', $transaction) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir esta transação?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="px-2 py-1 rounded bg-red-600 hover:bg-red-700 text-white font-bold transition shadow flex items-center gap-1" title="Excluir"><i class="fa-solid fa-trash"></i> Excluir</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <h3 class="mt-2 text-lg font-bold text-white">Nenhuma transação</h3>
                        <p class="mt-1 text-sm text-gray-300">Comece registrando suas primeiras transações.</p>
                        <div class="mt-6">
