...
<div class="container mx-auto py-6">
    ...
    <form action="{{ route('faturas.store') }}" method="POST" class="max-w-4xl bg-white p-6 rounded shadow">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="text" name="invoice_number" class="w-full border px-3 py-2 rounded" value="{{ old('invoice_number') }}" placeholder="Deixe em branco para gerar automaticamente">
            </div>
            <div>
                <label class="block mb-1 font-semibold">Data de Emissão</label>
                <input type="date" name="issue_date" class="w-full border px-3 py-2 rounded" value="{{ old('issue_date', date('Y-m-d')) }}">
            </div>
            <div>
                <label class="block mb-1 font-semibold">Data de Vencimento</label>
                <input type="date" name="due_date" class="w-full border px-3 py-2 rounded" value="{{ old('due_date', date('Y-m-d', strtotime('+30 days'))) }}">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <input type="number" step="0.01" name="subtotal" id="subtotal" class="w-full border px-3 py-2 rounded bg-gray-50" readonly>
            </div>
            <div>
                <label class="block mb-1 font-semibold">Taxa de Imposto (%)</label>
                <input type="number" step="0.01" name="tax_rate" id="tax_rate" class="w-full border px-3 py-2 rounded" value="{{ old('tax_rate', 0) }}">
            </div>
            <div>
                <label class="block mb-1 font-semibold">Valor do Imposto</label>
                <input type="number" step="0.01" name="tax_amount" id="tax_amount" class="w-full border px-3 py-2 rounded bg-gray-50" readonly>
            </div>
        </div>

        <div class="mb-6">
            <label class="block mb-1 font-semibold">Total</label>
            <input type="number" step="0.01" name="total" id="total" class="w-full border px-3 py-2 rounded text-xl font-bold bg-blue-50" value="{{ old('total') }}" required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <select name="status" class="w-full border px-3 py-2 rounded" required>
                    ...
                    <option value="paid" {{ old('status') == 'paid' ? 'selected' : '' }}>Paga</option>
                    <option value="overdue" {{ old('status') == 'overdue' ? 'selected' : '' }}>Vencida</option>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <textarea name="notes" class="w-full border px-3 py-2 rounded" rows="3">{{ old('notes') }}</textarea>
            </div>
            <div>
                <label class="block mb-1 font-semibold">Termos</label>
                <textarea name="terms" class="w-full border px-3 py-2 rounded" rows="3">{{ old('terms') }}</textarea>
            </div>
        </div>

        <div class="flex gap-2">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Salvar Fatura</button>
            <a href="{{ route('faturas.index') }}" class="bg-gray-300 px-6 py-2 rounded hover:bg-gray-400">Cancelar</a>
        </div>
    </form>
</div>
...
// Adicionar event listeners ao primeiro serviço
